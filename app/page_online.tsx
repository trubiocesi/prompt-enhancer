...
export default async function Home() {
  ...
  if (!session) {
    // No session → send the user to the login page
    redirect('/login');
  }

  // User is signed in → render your existing client component
...
+    await supabase
+      .from('prompts')
+      .insert({ user_id: userId, raw, enhanced: accumulated });
+    // prepend locally for instant UI update
+    setHistory([{ raw, enhanced: accumulated, timestamp: new Date().toISOString() }, ...history]);
    // ───────────────────────────────────────────────────
  }  
